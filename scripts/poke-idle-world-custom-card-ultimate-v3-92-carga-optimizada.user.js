// ==UserScript==
// @name         Poke Idle World - Custom Card Ultimate v3.92 (Carga optimizada)
...
(function() {
    ...
    function applyScriptScales() {
        ...
        lastEventSyncAt = 0;
        positionScriptSettingsButton(document.getElementById('custom-card-settings-btn'));
    ...
    GM_addStyle(`
        ...
        #custom-card-settings-btn {
            ...
            min-height:25px !important;padding:4px 7px !important;border:1px solid #334b60 !important;border-radius:5px !important;
            background:#0d131a !important;color:#dbeafe !important;box-shadow:0 3px 8px #0008 !important;
            font:800 9px/1 'Segoe UI',sans-serif !important;letter-spacing:.02em !important;white-space:nowrap !important;cursor:pointer !important;
        }
        ...
        @media (max-width:700px) {
            #custom-card-settings-btn { min-height:23px !important;padding:3px 6px !important;font-size:8px !important; }
    ...
    function positionScriptSettingsButton(button) {
        ...
        if (helperButton && helperButton.getClientRects().length && getComputedStyle(helperButton).display !== 'none') {
            ...
            const buttonWidth = buttonRect.width || 60;
            const buttonHeight = buttonRect.height || 25;
            const gap = 7;
            const desiredLeft = helperRect.left - buttonWidth - gap;
            const left = Math.max(8, Math.min(window.innerWidth - buttonWidth - 8, desiredLeft));
            top = Math.max(8, Math.min(window.innerHeight - buttonHeight - 8, helperRect.top + ((helperRect.height - buttonHeight) / 2)));
            right = Math.max(8, window.innerWidth - left - buttonWidth);
        }
